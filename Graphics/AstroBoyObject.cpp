...
void AstroBoyObject::setStartPos( const Vect & v)
{
	this->startPos = v;
...
void AstroBoyObject::setLightColor( const Vect & v)
{
	this->lightColor = v;

};

void AstroBoyObject::setLightPos( const Vect & v)
{
	this->lightPos = v;
};

void AstroBoyObject::setTest(const bool inTest)
{
	this -> test = inTest;
}

void AstroBoyObject::setScale(const float scaled)
{
	this -> xyz_scale = scaled;
}

void AstroBoyObject::setChangeScale(const float scaled)
{
	this -> xyz_change_scale = scaled;
}

void AstroBoyObject::setMaxScale(const float max)
{
	this -> maxScale = max;
}

void AstroBoyObject::setMinScale(const float min)
{
	this -> minScale = min;
}

void AstroBoyObject::setXAngle(const float xAngle)
{
	this -> x_angle = xAngle;
}

void AstroBoyObject::setYAngle(const float yAngle)
{
	this -> y_angle = yAngle;
}

void AstroBoyObject::setZAngle(const float zAngle)
{
	this -> z_angle = zAngle;
}

void AstroBoyObject::setXRotation(const float x_rotation)
{
	this -> x_rotate = x_rotation;
}

void AstroBoyObject::setYRotation(const float y_rotation)
{
	this -> y_rotate = y_rotation;
}

void AstroBoyObject::setZRotation(const float z_rotation)
{
	this -> z_rotate = z_rotation;
}

void AstroBoyObject::setXXRotation(const float x_rotation)
{
	this -> xx_rotate = x_rotation;
}

void AstroBoyObject::setYYRotation(const float y_rotation)
{
	this -> yy_rotate = y_rotation;
}

void AstroBoyObject::setZZRotation(const float z_rotation)
{
	this -> zz_rotate = z_rotation;
}

void AstroBoyObject::setXTranslation(const float x_translation)
{
	this -> x_translate = x_translation;
}

void AstroBoyObject::setYTranslation(const float y_translation)
{
	this -> y_translate = y_translation;
}

void AstroBoyObject::setZTranslation(const float z_translation)
{
...
void AstroBoyObject::transform( void )
{
	//update scale
...
void AstroBoyObject::setRenderState( void )
{
	// Bind the texture
   glBindTexture(GL_TEXTURE_2D, pAstroBoy->textureID);
...
void AstroBoyObject::draw( void )
{
	if (pAstroBoy -> getDuplicates() == false)
	{
		glBindVertexArray(pAstroBoy -> vao);

		/*  render primitives from array data */
		// (GLenum  mode,  GLsizei  count,  GLenum  type,  const GLvoid *  indices);
		glDrawElements(GL_TRIANGLES, pAstroBoy -> triangleCount * 3, GL_UNSIGNED_SHORT, 0);   //The starting point of the IBO
	}
	...
	else
	{
		...
		glDrawArrays(GL_TRIANGLES, 0, pAstroBoy -> getNumVerts());
	}
};